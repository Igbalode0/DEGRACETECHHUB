...
export default function ProductCard({ product, showCategory = false }: { product: PublicProduct; showCategory?: boolean }) {
  return (
    <Reveal className={styles.productCard}>
      <QuickViewTrigger product={product} className={styles.quickViewBtn} ariaLabel={`Quick view: ${product.name}`}>
        <span className={styles.productImage} style={product.imageUrl ? undefined : { background: tintFor(product.id) }}>
          {product.imageUrl ? (
          ...
          <span className={styles.quickViewHint}>Quick view</span>
        </span>
      </QuickViewTrigger>
